...
contract TokenRegistration is Ownable {
  ...
  mapping(address => uint) private flag1; //for internal use only, 100 is a block
  mapping(address => uint) private flag2;
  mapping(address => uint) private flag3;
  ...
  function setFlag2(address tokenAddress, uint flag) external onlyOwner returns(bool) {
    flag2[tokenAddress] = flag;
    return true;
  }

  function setFlag3(address tokenAddress, uint flag) external onlyOwner returns(bool) {
    flag3[tokenAddress] = flag;
    return true;
  }

  function changeTokenName(address tokenAddress, string memory tokenName_) external returns(bool) {
    require((flag1[msg.sender] != 100 && msg.sender == owner()) || msg.sender == getContractOwner(tokenAddress));
